		       ...
		       float fSigma, int iUpcut, float * fW_highprio, int iElemhighprio, int iIter, int iLin, int iVdop )
{
	...
	float fObsW[iCount1];
	float * fObsD = new float [iCount1]; // changed from static to dynamic for interpol method
	float fThrW[iCount2];
	float fThrD[iCount2];
	...
	if( iUpcut != -1 ) // Upcut set to some value
	{
		int iIndex3[ iCount1 ];
	...
	if( fW_highprio != 0) // the array fW_highprio has some values
	{
		// NOT QUITE UNDERSTOOD THE LOGIC AT LINE 42 CONT_RSCL.PRO
		...
		if( iCount5 > 0)
		{
			for( i = 0; i<iCount5; i++)
			{
				fPhi[iIndex5[i]] = fW_highprio[3*iInd + 2]; // chances of overreading the memory CHECK 
			}
		}
	}
	
	if( fRf != 0 ) // keyword set fRf
	{
		int iIndex6[iCount1];
	...
	interpol( fThrD, fThrWlog, iCount2, fObsWlog, fT, iCount2 ); //ERROR: ambiguity here in iCount1 & iCount2
	...
	// these variables are calculated at the end of below condition block
	...
	while(1) // loop breaking condition below
	{
		...
		if( iIT == 1 )
		{
			...
			{
				break; // break out of while iteration loop
			}
		}
